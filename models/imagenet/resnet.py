...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, base_width=64, stride=1, dilation=1, norm='bn', conv='normal', context='none',
                 ctx_ratio=0.0625, radix=1, stride_3x3=False, downsample=None):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, base_width=64, stride=1, dilation=1, norm='bn', conv='normal', context='none',
        ...
        if radix > 1 and (str3x3 > 1 or dilation > 1):
            ...
            self.avd_layer = None
        if radix > 1:
            self.conv2 = ops.SplAtConv2d(width, width, kernel_size=3, stride=str3x3, padding=dilation,
                                         dilation=dilation, bias=False, radix=radix)
        else:
    ...
    def __init__(self, inplanes, planes, base_width=64, stride=1, dilation=1, norm='bn', conv='normal', context='none',
        ...
        if radix > 1:
            self.conv1_2 = ops.SplAtConv2d(
                width, width, kernel_size=3, stride=stride, padding=dilation, dilation=dilation, bias=False, radix=radix
            )
        else:
        ...
        if radix > 1:
            self.conv2_2 = ops.SplAtConv2d(
                width // 2, width // 2, kernel_size=3, stride=stride, padding=dilation, dilation=dilation,
                bias=False, radix=radix
            )
            self.conv2_3 = ops.SplAtConv2d(
                width // 2, width // 2, kernel_size=3, stride=1, padding=dilation, dilation=dilation,
                bias=False, radix=radix
            )
        else:
            self.conv2_2 = conv_op(width // 2, width // 2, kernel_size=3, stride=stride, padding=dilation,
                                   dilation=dilation, bias=False)
            self.bn2_2 = make_norm(width // 2, norm=norm, an_k=10 if planes < 256 else 20)
            self.conv2_3 = conv_op(width // 2, width // 2, kernel_size=3, stride=1, padding=dilation,
                                   dilation=dilation, bias=False)
...
class ResNet(nn.Module):
    def __init__(self, bottleneck=True, aligned=False, use_3x3x3stem=False, stride_3x3=False, avg_down=False,
                 stem_width=64, base_width=64, layers=(3, 4, 6, 3), norm='bn',
                 stage_with_conv=('normal', 'normal', 'normal', 'normal'),
                 stage_with_context=('none', 'none', 'none', 'none'), ctx_ratio=16, radix=1,
                 num_classes=1000):
        """ Constructor
        Args:
