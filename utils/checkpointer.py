...
def load_weights(model, weights_path, use_weights_once=False, local_rank=0):
    ...
    model_state_dict, mismatch_keys = align_and_update_state_dicts(model_state_dict, weights_dict, use_weights_once, -1)
    ...
    def load_model(self, model, convert_conv1=False, use_weights_once=False):
        if self.resume:
            ...
            model_state_dict, self.mismatch_keys = align_and_update_state_dicts(model_state_dict, weights_dict,
                                                                                use_weights_once, self.local_rank)
            model.load_state_dict(model_state_dict)
...
def align_and_update_state_dicts(model_state_dict, weights_dict, use_weights_once=False, local_rank=0):
    """
    This function is taken from the maskrcnn_benchmark repo.
    It can be seen here:
    https://github.com/facebookresearch/maskrcnn-benchmark/blob/master/maskrcnn_benchmark/utils/model_serialization.py
