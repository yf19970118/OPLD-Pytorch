    ...
    with open(destfile, 'w') as f_out:
        ...
        for file in filenames:
            ...
            for obj in objects:
                ...
                # polygon:(8元素列表)
                quad = obj['poly']
                ctr_x, ctr_y = get_rotate_ctr(quad)
                quad.append(ctr_x)
                quad.append(ctr_y)
                single_obj['area'] = obj['area']
                single_obj['category_id'] = cls_names.index(obj['name']) + 1
                single_obj['segmentation'] = []
                single_obj['segmentation'].append(quad)
...
def get_rotate_ctr(quad):
    quad_box = [(xi, yi) for xi, yi in zip(quad[0::2], quad[1::2])]
    rbbox = cv2.minAreaRect(np.array(quad_box))
    ctr_x, ctr_y = rbbox[0]
    return int(ctr_x), int(ctr_y)
