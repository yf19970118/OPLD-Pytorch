            ...
            for obj in objects:
                ...
                if half_iou == 1:
                    ...
                    if len(out_poly) < 4:
                        ...
                        continue
                    # out_poly2 = []
                    # for i in range(len(out_poly)):          # （x1, y1, x2, y2, x3, y3, x4, y4）
                    ...
                    if len(out_poly) == 4:
                        poly_in_patch = change_order(obj['poly'], out_poly)
                    elif len(out_poly) == 5:
                        poly_in_patch = five_poly_to_four(obj['poly'], out_poly)
                    # polyInsub = polyorig2sub(left, up, poly_in_patch)
                    polyInsub = convert_patch_quad(poly_in_patch, left, up)
...
def five_poly_to_four(ori_poly, poly):
    distances = [cal_line_length(poly[i], poly[i + 1]) for i in range(len(poly) - 1)]
    distances.append(cal_line_length(poly[0], poly[4]))
    min_dis = np.array(distances).argsort()[0]
    four_poly = []
    count = 0

    while count < 5:
        if count == min_dis:
            x = int((poly[count][0] + poly[(count + 1) % 5][0]) / 2)
            y = int((poly[count][1] + poly[(count + 1) % 5][1]) / 2)
            four_poly.append((x, y))
            count = count + 1
        elif count == (min_dis + 1) % 5:
            count = count + 1
            continue
        else:
            x = int(poly[count][0])
            ...
            four_poly.append((x, y))
            count = count + 1
    out_poly = change_order(ori_poly, four_poly)
    return out_poly


def change_order(ori_poly, poly_list):
    out_poly = copy.deepcopy(poly_list)
    for i, point in enumerate(poly_list):
        if point in ori_poly:
            index = ori_poly.index(point)
            out_poly[index] = point
            for j in range(1, 4):
                out_poly[(index + j) % 4] = poly_list[(i + j) % 4]
            break
    return np.array(out_poly).astype(int)


def convert_patch_quad(poly, x1, y1):
    quad = [None for i in range(8)]
    polygon = [poly[i] for i in range(len(poly))]
    polygon = np.array(polygon)
    x = (polygon[:, 0] - x1).astype(int)
    y = (polygon[:, 1] - y1).astype(int)
    quad[0::2] = x
    quad[1::2] = y
    return quad
