...
from rcnn.modeling.opld.opld import OPLD
...
class Generalized_RCNN(nn.Module):
    def __init__(self, is_train=True):
        ...
        if cfg.MODEL.OPLD_ON:
            self.OPLD = OPLD(self.dim_in, self.spatial_scale)
        self._init_modules()
    ...
    def forward(self, images, targets=None):
        ...
        if cfg.MODEL.OPLD_ON:
            x, result, loss_grid = self.OPLD(conv_features, result, targets)
    ...
    def opld_net(self, conv_features, result, targets=None):
        if len(result[0]) == 0:
            return result
        with torch.no_grad():
            x, result, loss_grid = self.OPLD(conv_features, result, targets)
